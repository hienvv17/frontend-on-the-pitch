...
export default function TimeSlotSelector({ disabledSlots = [], open, onClose, ...rest }: TimeSlotSelectorProps) {
  ...
  const toggleSlot = (slot: Date) => {
    ...
    if (rest.selectedSlots.length === 0) {
      ...
      const range = timeSlots.slice(from, to + 1).filter(s => !isSlotDisabled(s));
  ...
  const handleContinue = () => {
    ...
    const endTimeValue = rest.selectedSlots[rest.selectedSlots.length - 1].toTimeString().slice(0, 5);
